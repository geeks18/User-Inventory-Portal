...
export class InventoryComponent implements OnInit {
  displayedColumns = ['title', 'name', 'content','url',  'actions'];
  ...
  startEdit(i: number, id: string, title: string, name: string, url: string, version: string, content: string) {
    this.id = id;
    // index row is used just for debugging proposes and can be removed
    this.index = i;
    console.log(this.index);
    const dialogRef = this.dialog.open(EditDialogComponent, {
      data: {id: id, title: title, name: name, url: url, version: version, content: content}
    });
  ...
  deleteItem(i: number, id: string, title: string, name: string, url: string) {
    this.index = i;
    this.id = id;
    const dialogRef = this.dialog.open(DeleteDialogComponent, {
      data: {id: id, title: title, name: name, url: url}
    });
  ...
  connect(): Observable<Item[]> {
    ...
    return merge(...displayDataChanges).pipe(map( () => {
        ...
        this.filteredData = this._exampleDatabase.data.slice().filter((item: Item) => {
          const searchStr = (item.title + item.url + item.name).toLowerCase();
